...
$access = checkPageAccess("greeting_screen");
?>
            ...
            <div class="container bootstrap snippets bootdeys p-4" >
                <div class="col-md-12">
                    <div class=" block-data timeline-centered timeline-sm" data-status="known">  
                        <?php parcxGreetingScreen(array("task"=>"11","edit"=>$access["edit"],"delete"=>$access["delete"])) ?>
                    </div>
                    
                    <!--<div class=" block-data timeline-centered timeline-sm" data-status="un-known" style="display:none;">
                        <?php parcxGreetingScreen(array("task"=>"11","type"=>2,"edit"=>$access["edit"],"delete"=>$access["delete"])) ?>         
                    </div>-->
                </div>
            </div>
        </div> 
...
<script type="text/javascript">
	...
	var known_customer = $(this).attr("data-customer");        
...
console.log(result);       
    ...
    {
        stage_id = $(this).attr("data-id");
	known_customer = $(this).attr("data-customer"); 
        schedule = '1';
        showform();        
        //showform(stage_id, 1);        
    });
...
//                });
  ...
  $(document).on("click", ".enable-disable-btn", function ()
	...
	var banColor = ['#ffffff','#000000','#F0FFF0', '#F5FFFA', '#F0FFFF', '#F5F5F5', '#FFFAFA', '#FFF5E', '#F5F5DC', '#F8F8FF', '#FFFAF0', '#FFFFF0','#FCFCFC'];
	var color = this.value;
	if( $.inArray( color, banColor ) >= 0 ){
	    	$("#info-message").html("This color is not allowed");
        	$("#info-modal").modal('show');
	}
	else{
		var data = {};
