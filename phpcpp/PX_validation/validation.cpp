...
void showUsersList()
    ...
    try
        ...
        if(res->rowsCount()>0)
            ...
            Php::out<<"<tr>"<<endl;
            Php::out<<"<th>Account name</th>"<<endl;
            Php::out<<"<th>Email</th>"<<endl; 
            Php::out<<"<th>Carpark</th>"<<endl;
            Php::out<<"<th>Valid From</th>"<<endl;       
            Php::out<<"<th>Valid To</th>"<<endl;           
            Php::out<<"<th>Products</th>"<<endl;           
            Php::out<<"<th></th>"<<endl;		           
        ...
        while(res->next())
            ...
            Php::out<<"<td>"+res->getString("email")+"</td>"<<endl;  
            if(res->getString("carparks")!="")
                {
                res2=stmt->executeQuery("select group_concat(carpark_name) as carparks from system_carparks where carpark_number in(" + res->getString("carparks") + ")");
                if (res2->next())
                    Php::out<<"<td>"+res2->getString("carparks")+"</td>"<<endl;
                else
                    Php::out<<"<td>"+res->getString("carparks")+"</td>"<<endl;
                delete res2;
                }
            else
                 Php::out<<"<td>"+res->getString("carparks")+"</td>"<<endl;
            ...
            if(res->getString("products")!="")
                {
                res2=stmt->executeQuery("select group_concat(product_name) as products from system_products where product_id in(" + res->getString("products") + ")");
                if (res2->next())
                    Php::out<<"<td>"+res2->getString("products")+"</td>"<<endl;
                else
                    Php::out<<"<td>"+res->getString("products")+"</td>"<<endl;
                delete res2;
                }
            else
                Php::out<<"<td>"+res->getString("products")+"</td>"<<endl;
            ...
            if(res->getInt("status")==1)
                Php::out << "<button type='button' class='btn btn-danger btn-enable-disable-validator' title='Disable' data-text='Disable'><i class='fas fa-stop-circle'></i></button>"<< std::endl;
            else
                Php::out << "<button type='button' class='btn btn-success btn-enable-disable-validator' title='Enable' data-text='Enable'><i class='fas fa-play-circle'></i></button>"<< std::endl;
            
            Php::out << "<button type='button' class='btn btn-info btn-edit-validator' title='Edit'><i class='fas fa-edit'></i></button>"<< std::endl;            
            
            Php::out << "<button type='button' class='btn btn-info validator-change-password' title='Change Password'><i class='fas fa-key'></i></button>"<< std::endl;            
            Php::out << "</td>"<< std::endl;
            Php::out<<"</tr>"<<endl;	
            }
...
Php::Value changePassword(Php::Value json)
    ...
    try
        ...
        con = mysqlConnect();
        stmt=con->createStatement();
        query="select user_id from system_validators where user_id="+id+" and password=md5('"+current_password+"')";
        res=stmt->executeQuery(query);
        if(res->next())
            {
            query="update system_validators set password=md5('"+new_password+"') where user_id="+id;
...
Php::Value parcxValidation(Php::Parameters &params) {
    ...
    switch (task) {
        ...
        case 15:validationTransactionReport(data);
            break;
        case 16:response=changePassword(data);
            break;
