...
void showCarparkList(Php::Value data) {
    try {
	int edit_access = data["edit"];
	int delete_access = data["delete"];
        con = General.mysqlConnect(ServerDB);
...
void addParcxV2UserActivity(Php::Parameters &params)
    {
    try
        {
...
extern "C" {
    ...
    PHPCPP_EXPORT void *get_module() {
        static Php::Extension extension("PX_V2_Settings", "1.0");
        extension.add<parcxV2Settings>("parcxV2Settings");
        extension.add<addParcxV2UserActivity>("addParcxV2UserActivity");
        return extension;
    }
}
